...
export default function SearchOtherProfiles() {
  ...
  const getProfileFilters = useSelector(profileFilters);
  ...
  useEffect(() => {
    dispatch(updateProfileSearch(debouncedNewsFeedSearch));
  }, [debouncedNewsFeedSearch]);

  useEffect(() => {
    if (getProfileFilters.searchData === '') {
      setNewsFeedSearch('');
    }
  }, [getProfileFilters.searchData]);
