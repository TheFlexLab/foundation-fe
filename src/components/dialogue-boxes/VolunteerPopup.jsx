...
const CertificationsPopup = ({
  ...
  const [existingData, setExistingData] = useState(selectedBadge?.personal?.volunteer || []);
    ...
    try {
      ...
      if (addBadge.status === 200) {
        if (field1Data.button) {
          const dataSaved = await api.post(`/addBadge/volunteer/add`, {
  ...
  const searchRoles = async () => {
    try {
      const jb = await api.post(`search/searchVolunteer/?name=${query}`);
      ...
      setRoles([{ id: `${Date.now()}-${Math.floor(Math.random() * 10000)}`, name: query, button: true }]);
    }
  };
          ...
          {!addAnotherForm ? (
            <div className="mx-3 flex flex-col gap-[2px] tablet:mx-[40px] tablet:gap-[5px]">
              <h1 className="py-3 text-[12px] font-medium leading-[13.56px] text-gray-1 dark:text-white-400 tablet:pb-[13px] tablet:text-[16px] tablet:leading-normal">
                Strengthen your verification by adding volunteer experience to your profile and highlight the causes you
                support.
              </h1>
  ...
  return (
    <PopUp open={isPopup} handleClose={handleClose} title={title} logo={logo}>
      {title === 'Volunteer' && renderWorkField(role, organization, startDate, endDate)}
    </PopUp>
  );
};
